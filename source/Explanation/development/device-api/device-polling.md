...
substitutions:
  image18: |-
    ```{image} device-polling/ThreadsManagement.png
    ```
---
...
and *attr_min_poll_period*. The property min_poll_period (ms)
...
*trigger_cmd_polling()* method (or *trigger_attr_polling()* method)
...
timestamp. In order to be read with the *command_inout_history* or
...
*fill_cmd_polling_buffer()* method (or
*fill_attr_polling_buffer()* method) of the Util class. For command,
