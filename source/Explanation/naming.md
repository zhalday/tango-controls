...
experiment they belong to. At ESRF, some of the machine
...
usage like device servers without database or device access for multi
control systems. With the naming schema, we must also be able to name
...
name, the device alias name and the device server name are all **case insensitive**.
