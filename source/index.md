...
- {doc}`Tutorials/index` provides information for **Developers** that comes in handy when developing {term}`Device Servers <device server>`, {term}`Devices <device>` and client applications.
