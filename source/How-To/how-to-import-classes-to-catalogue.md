...
[import script](https://gitlab.com/tango-controls/dsc-import).
...
3. Clone the import utility with git:
   : - {command}`git clone https://gitlab.com/tango-controls/dsc.git`
...
6. Update variables in a file {file}`dsc_import_utility.py` to reflect your environment:
...
7. run with a command: {command}`python dsc_import_utility.py`
